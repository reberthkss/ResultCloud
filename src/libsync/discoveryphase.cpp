...
void DiscoverySingleLocalDirectoryJob::run() {
    ...
    if (!dh) {
        ...
        QString errorString = tr("Erro ao abrir o diretório %1").arg(localPath);
        if (errno == EACCES) {
            errorString = tr("Diretório não acessível no cliente, permissão negada");
            emit finishedNonFatalError(errorString);
            return;
        } else if (errno == ENOENT) {
            errorString = tr("Diretório não encontrado: %1").arg(localPath);
    ...
    while (auto dirent = csync_vio_local_readdir(dh, _vfs)) {
        ...
        if (state.invalidChars > 0 || state.remainingChars > 0) {
            ...
            item->_errorString = tr("A codificação do nome do arquivo não é válida");
    ...
    if (errno != 0) {
        ...
        emit finishedFatalError(tr("Erro ao ler o diretório %1").arg(localPath));
        return;
    }
